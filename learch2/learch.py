...
class Learch2D(Learch):
    ...
    def __init__(self,
                 nb_points, centers, sigma, paths, starts, targets, pixel_map):
    ...
    def initialize_mydata(self):
        ...
        self.costmap = get_costmap(
            self.nb_points, self.centers, self.sigma, self.w)
...
def show_multiple_maps(costmaps, workspace):
    if show_result:
        ...
        viewer = render.WorkspaceDrawer(
            workspace, wait_for_keyboard=True, rows=r, cols=c)
...
for i in range(nb_samples):
    ...
    try:
        ...
        path = converter.dijkstra_on_map(
            np.exp(original_costmap), s[0], s[1], t[0], t[1])
    except Exception as e:
        print("Exception")
