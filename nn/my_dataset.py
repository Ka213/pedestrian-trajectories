...
def load_workspaces_from_file(filename='workspaces_1k_small.hdf5'):
    ...
    for k, phi in enumerate(data_ws.datasets):
        workspaces[k] = phi
    return workspaces
...
class CostmapDataset(object):
    ...
    def reshape_data_to_tensors(self):
        ...
        def reshape_targets_to_tensor(data):
            data = data.reshape(data.shape[0], data.shape[1] * data.shape[2])
            print("target shape", data.shape)
            return data

        self.train_inputs = reshape_inputs_to_tensor(self.train_inputs)
        self.train_targets = reshape_targets_to_tensor(self.train_targets)
        self.test_inputs = reshape_inputs_to_tensor(self.test_inputs)
        self.test_targets = reshape_targets_to_tensor(self.test_targets)
