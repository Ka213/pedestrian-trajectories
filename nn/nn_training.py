...
# Costmaps
costmaps = get_dataset_id(data_id=datafile)
...
if DRAW or SAVE_TO_FILE:
    ...
    for i in range(NUM_TEST_IMG):
        a[0][i].imshow(test_view_data_inputs[i].reshape(PIXELS, PIXELS),
                       cmap=cm.magma)
        a[0][i].set_xticks(())
        a[0][i].set_yticks(())

        # original data (first row) for viewing
        a[1][i].imshow(test_view_data_targets[i].reshape(PIXELS, PIXELS),
                       cmap=cm.magma)
        a[1][i].set_xticks(())
        a[1][i].set_yticks(())
...
for step in range(BATCHES):
    ...
    save_path = saver.save(sess, home + '/../data/tf_models/' + datafile
                           + '/model.ckpt')
