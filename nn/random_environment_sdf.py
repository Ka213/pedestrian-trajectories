...
def random_environments(opt):
    ...
    return data, workspaces, datap, dataep


def get_dataset_id(data_id):
    options_data = dataset_sdf.get_yaml_options()
    options = dict_to_object(options_data[data_id])
    filename = options.filename + "." + options.type
    filepath = dataset_sdf.learning_data_dir() + os.sep + filename
    if os.path.exists(filepath) and os.path.isfile(filepath):
        data = dataset_sdf.CostmapDataset(filename)
        ...
        dataset_sdf.write_dictionary_to_file(datasets, filename)
        dataset_sdf.write_dictionary_to_file(
            workspaces, options.workspaces + "." + options.type)
        dataset_sdf.save_paths_to_file(demonstrations, options.trajectories +
                                       "." + options.type)
        dataset_sdf.write_data_to_file(ep, options.endpoints + "." + options.type)
...
class RandomEnvironmentOptions:
    ...
    def get_options(self):
        ...
        if self._use_parser:
            ...
            options = dataset_sdf.get_yaml_options()[self._dataset_id]
...
if __name__ == '__main__':
    ...
    dataset_paramerters = dict_to_object(
        dataset_sdf.get_yaml_options()[options.dataset_id])
