...
class Learch_Loss_Aug_Esf(Learning):
    """ Implements the learch variant over multiple environments """
    ...
    def __init__(self, nb_points, nb_rbfs, sigma, workspace):
        ...
        self._learning_rate = 1  # 0.1 with normalized gradient
    ...
    def add_environment(self, centers, paths, starts, targets):
        ...
        C = Learch_Loss_Aug_Esf.Instance(phi, paths, starts, targets,
                                         self.workspace)
    ...
    class Instance(Learning.Instance):
        ...
        def planning(self):
            """ Compute the data set d where the cost function has to
                increase/decrease
            """
            d1 = np.empty((3, 0))
            d3 = np.empty((3, 0))
            for i, trajectory in enumerate(self.sample_trajectories):
                ...
                try:
                    o = get_expected_edge_frequency(map, self._N,
                                                    self.phi.shape[1],
                                                    self.sample_starts,
                                                    self.sample_targets,
                                                    self.workspace)
                    ...
                    raise
                # Add the states of with the loss augmented
                # expected state frequency to d
